...
@test "recovery partial failures: some SAs deleted successfully, others fail - should continue and verify" {
	...
	setup_vpn_xfrm_recovery_fixture "${TEST_PEER_IP}" 3 "partial_failure" 'TIER3_THRESHOLD=5' || fail "Fixture setup failed"
...
@test "recovery partial failures: multiple recovery actions triggered simultaneously - should be prevented by lockfile" {
	...
	setup_vpn_at_tier_fixture 2 "${TEST_PEER_IP}" 'ENABLE_XFRM_RECOVERY=1' 'ENABLE_NETWORK_PARTITION_CHECK=0' 'LOCKFILE_TIMEOUT=60' || fail "Fixture setup failed"
