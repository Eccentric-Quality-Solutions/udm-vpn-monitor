...
4. **Set up git hooks**:
   ...
   **Note**: The hooks will warn if ShellCheck or shfmt are not installed, but will still allow the commit. For best results, install both tools (see [Required Tools](#required-tools) below).

   Install packages (`udm-vpn-monitor.zip` / `.tar.gz`) are **not** regenerated by the hook. They are built by GitHub Actions (workflow artifacts on push/PR; Release assets on `v*` tags). For a local package: `./scripts/prepare_install_package.sh` (optionally `--tar`).
...
**Operating mode control** (`lib/control/`, `vpn-monitor-control.sh`, `manage/control-remote-udm.sh`): local and remote start/stop/pause/observe-only for the monitor. State file: `state/operating_mode` (see `docs/reference/STATE_SYSTEM.md`). Tests: `tests/test_operating_mode.sh`, `tests/test_remote_control.sh`.
...
This repository includes a pre-commit hook that runs code quality checks (ShellCheck and shfmt) on staged shell scripts before each commit.

Install packages (`udm-vpn-monitor.zip` / `.tar.gz`) are built by GitHub Actions, not committed:
- **Push / PR**: `.github/workflows/build-package.yml` uploads both archives as the `udm-vpn-monitor-packages` workflow artifact (30-day retention).
- **Tags (`v*`)**: `.github/workflows/release.yml` attaches both archives to a GitHub Release.
- **Local**: `./scripts/prepare_install_package.sh` (and `--tar`). Deploy scripts still build the zip locally if it is missing.
...
**Note**: The hook will warn if ShellCheck or shfmt are not installed, but will still allow the commit. For best results and to catch issues early, install both tools (see [Required Tools](#required-tools) above).
