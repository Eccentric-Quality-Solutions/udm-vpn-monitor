...
check_xfrm_primary() {
	...
	check_xfrm_status "$peer_ip" "$internal_peer_ip" "$location_name" "$diagnostic_var" "$sa_exists_var" "$xfrm_output_var"
}
...
check_ipsec_fallback() {
	...
	check_ipsec_status "$peer_ip" "$location_name" "$diagnostic_var"
}
