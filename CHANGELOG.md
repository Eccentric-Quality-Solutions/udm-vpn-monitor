...
- **BREAKING (safe-by-default operating mode):** Fresh installs and a missing `state/operating_mode` file now default to `observe-only` (detection and logging only; no recovery) instead of `running`. Use `vpn-monitor-control.sh start` (or `manage/control-remote-udm.sh … start`) to enable recovery. On-box install/upgrade leaves an existing `operating_mode` file unchanged; **fleet deploy** (uninstall → install with default `--remove-state`) resets mode to `observe-only`. Keepalive is unchanged by observe-only. Acceptance checklist: `docs/scripts/CONTROL_PLANE_ACCEPTANCE.md` (fleet deploy skips interactive tail by default; avoid `start` while clearing pause during stage-1).
- **Fleet deploy default**: `deploy-to-udms.sh` defaults to non-interactive deploy (auto-record registry). Opt into the old interactive `tail -f` + y/n confirm with `--tail-follow`. `--skip-tail` remains available and is now the default behavior.
- **Fleet package default path**: `deploy-to-udms.sh` default `--file` is `${REPO_ROOT}/udm-vpn-monitor.zip` (alongside `prepare_install_package.sh` output) instead of `/tmp/udm-vpn-monitor.zip`.
- **Install package no longer in commits**: Pre-commit no longer runs `prepare_install_package.sh` or stages `udm-vpn-monitor.zip`. Packages are gitignored; build locally or download from Actions/Releases.
