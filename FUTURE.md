...
- **Module unit tests (remaining gaps)** — Dedicated coverage exists for `recovery_state`, recovery orchestration helpers, `config_loading`, `config_validation`, and location parsing. Still thin: `recovery_verification.sh`, `ipsec_recovery.sh`, `xfrm_recovery.sh`, `config_defaults.sh` (mostly exercised via broader recovery/config integration tests).

- **Testing cleanup (low churn until touched)** — Refactor `test_recovery_multi_location_partial.sh` (and similar) to use `tests/fixtures/vpn_multi_location.bash`. Add regression test for vpn-monitor-wrapper single-instance lock (second instance exits). Improve `tests/test_lockfile.sh` case that relies on read-only dirs (timing/FS-dependent).
...
- **DNS resolution enhancements** — Cache TTL (today: process-lifetime cache only); partial resolve for multi-IP names (today: first IP only); retries on transient failure.
