...
- Enhanced recovery type analysis in log reports
    ...
    - Note: Basic recovery type distinction implemented in root `analyze-logs.sh` (2026-01-06)
...
- Location getter exit codes (optional callers)
  - `get_location_external_ip` and `get_location_internal_ips` return 1 = not found, 7 (EXIT_MALFORMED_DATA) = malformed data. Validation and route-setup code already treat 7 as error. vpn-monitor.sh and keepalive use `2>/dev/null || echo ""` and do not distinguish; could log "malformed location data" when status is 7 for easier debugging (low priority).
  - Optional test: validate_config fails with clear message when LOCATIONS contains malformed entry (inject LOCATIONS["X"]="garbage" then run validate_config; would require test to call validate_config after parse and then mutate LOCATIONS).
...
- Refactor multi-location tests to use `setup_vpn_multi_location_fixture`
  - `test_recovery_multi_location_partial.sh` and similar multi-location tests currently build config/state manually
  - Could use `tests/fixtures/vpn_multi_location.bash` for consistent setup and less duplication
  - Low priority: existing tests work; refactor when touching those files

- Add module-level unit tests for recovery modules
  - Each recovery module in `lib/recovery/` (`recovery_verification.sh`, `recovery_state.sh`, `recovery_orchestration.sh`, `ipsec_recovery.sh`, `xfrm_recovery.sh`) could have focused unit tests
  ...
  - Pattern documented in `docs/reference/CODE_PATTERNS.md` under "Fake Mode Support" section
  - Current coverage:
...
- Add module-level unit tests for config modules
  - Each config module in `lib/config/` (`config_loading.sh`, `config_validation.sh`, `config_defaults.sh`, `location_parsing.sh`) could have focused unit tests
  - Would allow testing modules in isolation without full integration test setup
  - Low priority: existing integration tests cover functionality well
  - Benefit: Faster test execution, easier debugging of module-specific issues
...
- Add dedicated test for vpn-monitor-wrapper.sh single-instance lock
    - Wrapper uses flock/mkdir for atomic lock; no test directly exercises the lock behavior (second instance exits when first holds lock)
    - `tests/test_install.sh` verifies wrapper is used in cron but doesn't test lock acquisition
    - Low priority: manual verification confirms behavior; lock pattern matches `lib/lockfile.sh`
    - Benefit: Regression protection if wrapper lock logic changes

- Improve lockfile removal failure test reliability
    - Current test "lockfile cleanup fails - lockfile removal fails" in `tests/test_lockfile.sh` attempts to make directory read-only during script execution
...
- Review and fix ip xfrm state mock patterns in tests
    ...
    - `execute_xfrm_state_command()` in `lib/detection/xfrm_detection.sh` tries `ip -s xfrm state` first, then falls back to `ip xfrm state`
    - Tests that mock `ip` need to handle both formats to work correctly
    - Found 43+ instances of old pattern `if [[ "$1" == "xfrm" ]] && [[ "$2" == "state" ]]` that may need updating
    - Helper functions like `mock_ip_xfrm_state()` in test helpers already handle both formats correctly
    - Low priority: Tests may still work if they don't call code that uses the -s path or if fallback is used
    - Action: Review tests that use inline `ip xfrm state` mocks and update to handle `-s` flag or use helper functions
    - Note: Fixed in `tests/test_detection_ping_optional.sh` 2026-01-28
...
- Remove redundant defensive fallbacks in state.sh and config.sh
    - Current state: `lib/state.sh` and `lib/config.sh` (compat layers) have defensive fallback constants for when constants.sh is sourced multiple times
    ...
    - Benefit: Reduces code duplication, simplifies maintenance
    - Cost: LOW - simple removal, but removes safety net if constants.sh is missing/corrupted
    - Priority: LOW - fallbacks don't hurt and provide safety net for edge cases
...
- Consider extracting sed script building into shared functions if duplication increases
    - Current state: Similar sed script building patterns exist across scripts in `scripts/anonymize/` (anonymize-firewall.sh, anonymize-ip-rules.sh, anonymize-logs.sh, anonymize-ipset.sh) using `lib/anonymize.sh`
